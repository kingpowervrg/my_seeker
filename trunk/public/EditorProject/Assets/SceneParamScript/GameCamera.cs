...
namespace SeekerGame
{
    public class GameCamera : MonoBehaviour
    {
        ...
        protected void OnHandSwipeHandler(Gesture gesture, float moveTime)
        {
            ...
            rotateTweener.Kill();
            Vector3 targetRotate = GameSceneCamera.transform.localEulerAngles + rotateValue * 0.1f * CameraManager.DragDelta;// + m_moveRotate;
            m_cacheRotate = targetRotate;
            
            rotateTweener = GameSceneCamera.transform.DOLocalRotate(targetRotate, moveTime).SetEase((Ease)CameraManager.ease).OnUpdate(OnRotateTweenerTweening).OnComplete(()=> {
