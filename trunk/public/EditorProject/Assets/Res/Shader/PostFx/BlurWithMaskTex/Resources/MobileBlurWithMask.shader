...
Shader "Hidden/PostFx/BlurWithMask" {
	Properties{
		_MainTex("Base (RGB)", 2D) = "white" {}
		_OriginTex("Origin (RGB)", 2D) = "black" {}
...
#include "UnityCG.cginc"
	...
	v2f_fade vertfade(appdata_img v)
	{
		v2f_fade o;
		o.pos = UnityObjectToClipPos(v.vertex);
	...
	fixed4 fragfade(v2f_fade i) : SV_Target
	{
		fixed4 color = tex2D(_MainTex, i.uv);
		fixed4 origin = tex2D(_OriginTex, i.uv);
		fixed4 blend = tex2D(_MaskTex, i.uv);
		fixed4 fogCol = tex2D(_BlendTex,i.uv);
		return lerp(origin, lerp(color,fogCol,fogCol.a),(1 - blend.r) * _factor);
		//return fogCol;
	}
	...
	half4 fragBlurSGX(v2f_withBlurCoordsSGX i) : SV_Target
	{
		...
		half4 fragSolidColor(v2f_img i) :SV_Target
	{
