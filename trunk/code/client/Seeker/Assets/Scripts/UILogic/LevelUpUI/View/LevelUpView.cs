...
{
    public class LevelUpView : BaseViewComponet<LevelUpUILogic>
    {
        #region 标题
        ...
        public void Refresh(SCPlayerUpLevel msg_)
        {
            m_data = msg_;
        ...
        {
            m_from_lvl_txt.Text = string.Format("Lv.{0}", m_data.OldLevel);
            m_to_lvl_txt.Text = string.Format("Lv.{0}", m_data.NewLevel);
            ...
            m_grid.EnsureSize<DropItemIcon>(m_data.Rewards.Count);

            for (int i = 0; i < m_grid.ChildCount; ++i)
            {
                ConfProp prop = ConfProp.Get(m_data.Rewards[i].PropId);

                m_grid.GetChild<DropItemIcon>(i).InitSprite(prop.icon, m_data.Rewards[i].Num, prop.id);
        ...
        protected virtual void OnClick(GameObject obj)
        {
            ...
            if (0 == m_data.UnLockBuildId)
                CurViewLogic().OnClose();
            else
                CurViewLogic().OnGoToMap(m_data.UnLockBuildId);
        }
    }
}
