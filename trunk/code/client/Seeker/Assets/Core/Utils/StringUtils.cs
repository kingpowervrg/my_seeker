...
namespace EngineCore
{
    public static class StringUtils
    {
        ...
        /// </summary>
        /// <param name="str"></param>
        /// <param name="invalidCharArray"></param>
        /// <returns></returns>
        public static bool IsValidString(string dstStr, char[] invalidCharArray = null)
        {
            invalidCharArray = invalidCharArray ?? InvalidCustomChars;
            HashSet<Char> invalidCharSet = new HashSet<char>(invalidCharArray);

            for (int i = 0; i < dstStr.Length; ++i)
            {
                if (invalidCharSet.Contains(dstStr[i]))
                    return false;
            }
        ...
        /// <param name="url"></param>
        /// <returns></returns>
        public static bool IsEnginshOrChineseStr(string inputStr)
        {
            string pattern = "[\u4e00-\u9fa5a-zA-Z0-9]*";
            Match match = Regex.Match(inputStr, pattern);

            return match != null && match.Value == inputStr;
        }
