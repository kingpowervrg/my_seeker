...
public class ConfigUtil
{
    ...
    {
        EngineCore.EngineCoreEvents.ResourceEvent.GetAssetEvent.SafeInvoke(name, OnLoadFile, EngineCore.LoadPriority.Default);
    }


    public static void ReleaseAsset(string name, UnityEngine.Object obj)
    {
        EngineCore.EngineCoreEvents.ResourceEvent.ReleaseAssetEvent.SafeInvoke(name, obj);
    }

    public static void RemoveAsset(string name)
    {
        EngineCore.EngineCoreEvents.ResourceEvent.RemoveAssetEvent.SafeInvoke(name, true, true);
    }
    ...
    public static void ReadColumnData(BinaryReader br, int col_cnt, List<int> dic_type, List<string> field_list)
    {
        for (int i = 0; i < col_cnt; i++)
        {
            ...
            else if (type.StartsWithFast("string"))
            {
                if (type.Contains("["))
                {
                    dic_type.Add(12);
            ...
            else if (type.StartsWithFast("float"))
            {
                if (type.Contains("["))
                {
                    dic_type.Add(13);
            ...
            else if (type.StartsWithFast("bool") || type.StartsWithFast("boolean"))
            {
                if (type.Contains("["))
                {
                    dic_type.Add(14);
            ...
            else if (type.StartsWithFast("short"))
            {
                if (type.Contains("["))
                {
                    dic_type.Add(15);
            ...
            else if (type.StartsWithFast("byte"))
            {
                if (type.Contains("["))
                {
                    dic_type.Add(16);
            ...
            else if (type.StartsWithFast("long"))
            {
                if (type.Contains("["))
                {
                    dic_type.Add(17);
