...
namespace EngineCore
{
    public class Audio
    {
        ...
        public Audio(AudioType audioType, AudioClip clip, bool loop, bool persist, float volume, float fadeInValue, float fadeOutValue, Transform sourceTransform)
        {
            ...
            Pitch = 1;
            StereoPan = 0;
            if (sourceTransform != null && sourceTransform != AudioModule.Instance.AudioRootObject.transform)
            {
                SpatialBlend = 1;
            }
            ReverbZoneMix = 1;
        ...
        private void CreateAudiosource()
        {
            if (sourceTransform == null)
                sourceTransform = AudioModule.Instance.AudioRootObject.transform;
            ...
            switch (Type)
            {
                case AudioType.Music:
                    AudioSource.outputAudioMixerGroup = EngineDelegateCore.DefaultBGMMixerGroup;
                    break;
                case AudioType.Sound:
                case AudioType.UISound:
                    AudioSource.outputAudioMixerGroup = EngineDelegateCore.DefaultEffectMixerGroup;
                    break;
            }
        ...
        public void Play(float volume)
        {
            ...
            if (Pooled)
            {
                ...
                bool restoredFromPool = AudioModule.Instance.RestoreAudioFromPool(Type, AudioID);
                if (!restoredFromPool)
                {
                    return;
                }
        ...
        public void Update()
        {
            ...
            switch (Type)
            {
                case AudioType.Music:
                    {
                        AudioSource.volume = Volume * AudioModule.Instance.GlobalMusicVolume * AudioModule.Instance.GlobalVolume;
                        break;
                    }
                case AudioType.Sound:
                    {
                        AudioSource.volume = Volume * AudioModule.Instance.GlobalSoundsVolume * AudioModule.Instance.GlobalVolume;
                        break;
                    }
                case AudioType.UISound:
                    {
                        AudioSource.volume = Volume * AudioModule.Instance.GlobalUISoundsVolume * AudioModule.Instance.GlobalVolume;
                        break;
                    }
            }
