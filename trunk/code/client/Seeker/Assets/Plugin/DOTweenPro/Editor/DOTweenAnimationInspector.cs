        ...
        override public void OnInspectorGUI()
        {
            ...
            if (playMode) {
                if (_runtimeEditMode) {
                    ...
                    GUILayout.Space(8);
                    GUILayout.Label("SkeletonAnimation Editor disabled while in play mode", EditorGUIUtils.wordWrapLabelStyle);
                    if (!_src.isActive) {
                        GUILayout.Label("This SkeletonAnimation has been toggled as inactive and won't be generated", EditorGUIUtils.wordWrapLabelStyle);
                        GUI.enabled = false;
                    }
            ...
            Undo.RecordObject(_src, "DOTween SkeletonAnimation");
...
//                _src.animationType = (DOTweenAnimationType)EditorGUILayout.EnumPopup(_src.animationType, EditorGUIUtils.popupButton);
                _src.isActive = EditorGUILayout.Toggle(new GUIContent("", "If unchecked, this SkeletonAnimation will not be created"), _src.isActive, GUILayout.Width(16));
            ...
            if (prevAnimType != _src.animationType) {
                // Set default optional values based on SkeletonAnimation type
            ...
            if (!_src.isValid) {
                ...
                GUILayout.Label("No valid Component was found for the selected SkeletonAnimation", EditorGUIUtils.wordWrapLabelStyle);
